...
q='scripts/whip-render-smoke.mjs';a=fs.readFileSync(q,'utf8').replace('X.lineWidth=14*p','X.lineWidth=15*p').replace('X.lineWidth=6*p','X.lineWidth=7*p').replace("if(state==='play')","const _soloHud=hud;").replace('top-layer + zoom-stable widths + extended lash life','post-world/pre-HUD layer + zoom-stable elastic lash + 0.2s visibility');fs.writeFileSync(q,a);
q='scripts/packed-smoke.mjs';a=fs.readFileSync(q,'utf8');let old="length>=3))throw Error(path+' refocus-safe animated rainbow whip failed')",neu="length>=2))throw Error(path+' refocus-safe animated rainbow whip failed')";if(!a.includes(old))throw Error('missing packed whip contract marker');a=a.replace(old,neu);fs.writeFileSync(q,a);
