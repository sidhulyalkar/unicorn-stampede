...
for(const s of ['u.ai=.28','u.tx=cl(','u.ty=cl(','showcaseStreetBase','showcaseBuildingBase','drawPeople=function'])if(!show.includes(s))throw Error('showcase marker missing: '+s);
if(/POST-JS13K SHOWCASE EDITION/.test(show+index))throw Error('obsolete edition branding leaked into player-facing build');
for(const s of ['zone===0','zone===1','zone===2','zone===3'])if(!show.includes(s))throw Error('four-world showcase marker missing: '+s);
console.log('showcase structural smoke: PASS anti-stuck + browser guard + four-world graphics + detailed people + clean player branding');
