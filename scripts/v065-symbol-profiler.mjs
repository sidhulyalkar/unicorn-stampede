...
function circles(s){let seek='X.beginPath();X.arc(',at=0;for(;;){let i=s.indexOf(seek,at);if(i<0)break;let o=i+seek.length-1,e=closeParen(s,o),tail=';X.fill();';if(e<0)break;if(s.slice(e+1,e+1+tail.length)!==tail){at=e+1;continue}let args=splitArgs(s.slice(o+1,e));if(args.length===5&&args[3]==='0'&&args[4]==='T'){let r=`townCircle(${args.slice(0,3).join(',')});`;s=s.slice(0,i)+r+s.slice(e+1+tail.length);at=i+r.length}else at=e+1}return s}
