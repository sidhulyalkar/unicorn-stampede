...
const cut={
 staticPeople:f=>(f.herd=rep(f.herd,'updatePeople(dt);',''),f),
 noPeople:f=>(f.herd=rep(f.herd,'updatePeople(dt);',''),f.render=rep(f.render,'drawPeople();',''),f.core=rep(f.core,'for(let i=0;i<18;i++)people.push(makePerson(R(1120,80),R(575,90)));',''),f.core=rep(f.core,'for(let i=0;i<76;i++)people.push(makePerson(R(3130,70),R(1700,90)));',''),f),
 musicLite:f=>(f.top10=rep(f.top10,"if(stamp>2&&n%2)tone(220+55*(n%4),.04,'triangle',.007)",''),f),
 noMusic:f=>(f.top10=rep(f.top10,'music(dt);',''),f),
 noUnload:f=>(f.core=rep(f.core,"globalThis.onbeforeunload=e=>state==='play'&&(e.preventDefault(),e.returnValue='');",''),f),
 noErrorRecovery:f=>(f.core=rep(f.core,"globalThis.onerror=()=>{state='title';paused=0;requestAnimationFrame(frame)};",''),f),
 simpleMarkers:f=>(f.polish=rep(f.polish,"if(!done){X.strokeStyle='#fff';X.lineWidth=2;X.beginPath();X.arc(sx,sy,r+6+Math.sin(clock*7)*2,0,T);X.stroke()}",''),f),
 noMarkerNames:f=>(f.polish=rep(f.polish,"X.fillStyle='#fff';text(l.n,sx,sy-20,10,'center')",''),f)
};
