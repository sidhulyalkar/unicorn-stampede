import{releaseSource}from'./release-prune.mjs';import{staticizeWrappers}from'./release-staticize.mjs';import{compactRepresentation}from'./release-represent.mjs';import{fuseRelease}from'./release-fuse.mjs';
...
export function submissionSource(raw){let s=releaseSource(raw);for(let a of cuts)s=remove(s,a);return fuseRelease(compactRepresentation(staticizeWrappers(s)))}
