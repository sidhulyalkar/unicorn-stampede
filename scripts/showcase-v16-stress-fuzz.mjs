...
for(let z0=0;z0<4;z0++)for(let m=0;m<4;m++)for(let seed=1;seed<=3;seed++){
  zone=z0;mode=m;paused=0;startLevel(1);const rnd=rng0((z0+1)*0x9e3779b1^(m+7)*0x85ebca6b^seed*0xc2b2ae35),pin=new Map(),seenActive=new Set([caps[0]]);let ended=0,switches=0,whips=0,dashes=0,localMaxPin=0;
  ...
  clearKeys();paused=1;scenarios.push({z:z0,m,seed,ended,switches,whips,dashes,activeCoverage:seenActive.size,maxPinned:localMaxPin,captured:cleaners.filter(c=>c.u>=0).length,boundary:{...showcaseBoundaries.stats}});if(failures.length>20)break;
}
return{scenarios,failures,totalTicks,totalSwitches,totalWhips,totalDashes,totalEnds,maxPinned};});
if(report.failures.length)throw Error(name+' v1.6 stress failures '+JSON.stringify(report.failures.slice(0,20)));if(report.scenarios.length!==48)throw Error(name+' stress scenario count '+report.scenarios.length);if(report.totalTicks<25000)throw Error(name+' stress run ended too early '+report.totalTicks);if(report.totalSwitches<700||report.totalWhips<400)throw Error(name+' stress action coverage too low '+JSON.stringify({switches:report.totalSwitches,whips:report.totalWhips}));if(errors.length)throw Error(name+' page errors '+errors.join(' | '));console.log(name+' showcase v1.6 deterministic stress: PASS '+JSON.stringify({scenarios:report.scenarios.length,ticks:report.totalTicks,switches:report.totalSwitches,whips:report.totalWhips,dashes:report.totalDashes,ends:report.totalEnds,maxPinnedFrames:report.maxPinned}));await browser.close()}}finally{server.close()}
