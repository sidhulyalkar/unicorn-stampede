...
edit('src/core.js','hot:0,order:0','hot:0,cool:0,order:0');
edit('src/core.js','u.hot=Math.max(0,u.hot-dt);u.frenzy=','u.hot=Math.max(0,u.hot-dt);u.cool=Math.max(0,u.cool-dt);u.frenzy=');
edit('src/herd.js','function distractForce(u){let ax=0','function distractForce(u){if(!u.cap&&u.cool)return[0,0];let ax=0');
edit('src/herd.js','}}}return[ax,ay]}','}}}if(hit&&!u.cap)u.cool=5;return[ax,ay]}');
...
edit('src/herd.js',"else{u.x-=u.vx*dt;u.y-=u.vy*dt;u.vx=u.vy=u.ai=0;if(o.t==='fountain'||o.t==='pond')", "else{let x=u.x-o.x-o.w/2,y=u.y-o.y-o.h/2;if(Math.abs(x/o.w)>Math.abs(y/o.h))u.x=o.x+(x>0?o.w+ex:-ex),u.vx=0;else u.y=o.y+(y>0?o.h+ex:-ex),u.vy=0;u.ai=0;if(o.t==='fountain'||o.t==='pond')");
edit('src/expansion.js','Math.hypot(q.x-u.x,q.y-u.y)<260+mode*20))u.distract=2','Math.hypot(q.x-u.x,q.y-u.y)<260+mode*20&&!u.cool))u.distract=2,u.cool=5');
console.log('staged spawn/scenery clearance + finite unattended distractions + aligned aggression + wall-slide projection');
