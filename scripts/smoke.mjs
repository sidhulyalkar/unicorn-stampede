...
{
 const {ev}=game();ev("startLevel(1);landWin=1;state='end';startLevel(1)");if(ev('zone')!==1||ev('cleaners.length')!==2||Math.abs(ev('cleaners[0].v'))<190)throw Error('washwater must remix cleanup pressure');ev("landWin=1;chains=2;painted=PW*PH*.65");if(ev('grade()')!==3)throw Error('washwater crowns');
}
{
 const {ev,click}=game();ev("startLevel(1);landWin=1;state='end';startLevel(1);landWin=1;state='end';startLevel(1)");if(ev('zone')!==2)throw Error('cloudtop unlock');click();if(ev('unis[caps[0]].tapT')>.39)throw Error('cloudtop tighter whip window');
}
{
 const {ev}=game();ev("S.s('ccC2',1);S.s('ccHeat',0);startLevel(1)");if(!ev('plus')||ev('heat')!==0||ev('cleaners.length')!==1||!ev("lmText.includes('HEAT 1')"))throw Error('Stampede+ Heat 1 unlock');if(Math.abs(ev('chaos()')-.28)>1e-6)throw Error('Heat 1 chaos target');ev('victoryT=.01;landWin=1;update(.02)');if(ev("S.g('ccHeat')")!==1)throw Error('winning must advance Heat');
}
{
 const {ev}=game();ev("S.s('ccC2',1);S.s('ccHeat',5);startLevel(1)");if(ev('heat')!==5||ev('cleaners.length')!==2||Math.abs(ev('chaos()')-.33)>1e-6)throw Error('Heat ladder must compound cleanup and chaos pressure');
}
let ws=fs.readFileSync('src/whip.js','utf8'),es=fs.readFileSync('src/expansion.js','utf8'),rs=fs.readFileSync('src/render.js','utf8'),hs=fs.readFileSync('src/herd.js','utf8');if(!ws.includes('X.arc(-7,7,14+i*7,.2,5.6)')||!ws.includes('quadraticCurveTo')||ws.includes('*700-56')||!ws.includes('whip.l*520'))throw Error('snap whip regression');if(!ws.includes("if(n<2){msg='WHIP 2X → DASH'")||ws.includes("msg='FOLLOW LESSON'"))throw Error('two-whip dash contract regression');if(!rs.includes('u.h+120')||!hs.includes('f.r+155')||es.includes("timeLeft+=6")||!es.includes('FINAL CLEANUP')||!es.includes('!cleaners.some(c=>c.u>=0)')||!es.includes("msg='RALLY '+n+'/'+k")||!es.includes("msg=w===2?'COMET JOINS")||!ws.includes("'CAPTURED '+c+'/3'"))throw Error('mane/distraction/end-state/takeover/staged-herd regression');
if(!fs.readFileSync('src/style.css','utf8').includes('cursor:none'))throw Error('native cursor not hidden');
console.log('headless v0.32 route handoff + staged herd + rally + explicit end states: PASS');
