...
for(let q of ['g=83>>k&1','8+(57>>k&1)*5',"'B$=C*MTA'[k]",'o.ln||N[k+5]','hsl(${o.hue} 42%'])if(!r.includes(q))throw Error('missing semantic storefront component: '+q);
for(let q of ["'O$#~*+%@'[k]",'g=k&1||k>3','hsl(${o.hue} 34%'])if(r.includes(q))throw Error('retired arbitrary storefront grammar survived: '+q);
...
console.log('v0.57 semantic storefront grammar: PASS 8 stable palettes + meaningful ASCII emblems + reusable gable/awning bitmasks; gameplay durability remains generation-owned');
