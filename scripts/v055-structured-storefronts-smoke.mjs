...
for(let q of ["y+h-65,w,10","for(let i=3;i--;)","i==1?'#432':'#bff'","y+h-48,36,i==1?48:28","y+h/3,44,'center'","y+h-68,13,'center'","'O$#~*'","'STARTUP'","'YOGA'","'FARMERS'","'#a34'","'#7ff'"])if(!r.includes(q))throw Error('missing v0.55 structured storefront contract: '+q);
for(let q of ["n=k+3,g=k&1||k>3","h-100-g*14","y+h-12,36,12"])if(r.includes(q))throw Error('retired facade clutter returned: '+q);
for(let q of ["h[i].x=400+i*460","cars=cars.filter(c=>!c.dir)","wind*(90+stamp*8)*dt"])if(!w.includes(q))throw Error('Cloudtop gameplay identity changed: '+q);
