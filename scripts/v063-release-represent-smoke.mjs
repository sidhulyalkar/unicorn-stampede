...
for(let a of ["document.getElementById('c')",'const MM=Math,C=',"function drawTownRoof(x,y,w,k,H,house)","X.textAlign=['left','center','right'][a]","o.type=['sine','square','sawtooth','triangle'][t]","C.addEventListener('mousemove',point);","e.code[0]=='K'?e.code[3]:e.code",'K.Space'])if(!s.includes(a))throw Error('representation contract missing: '+a);
...
console.log('v0.63 release representation: PASS quality spine retained, non-WASD keys preserved, safe compact forms active');
