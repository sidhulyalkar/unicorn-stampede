...
s=swap(s,"function harbor(){roads.splice(1,1);roads[0][2]=140;roads[1][2]=160;objs=objs.filter(o=>o.t!=='hedge');cars=cars.filter(c=>c.dir||c.x<1200||c.x>2e3);ups[2].x=ups[6].x=1630}","function harbor(){roads.splice(1,1);roads[0][2]=140;roads[1][2]=160;let h=objs.filter(o=>o.t==='hedge'),a=[0,270,730,930,1320,1560];for(let i=6;i--;)h[i].x=1540,h[i].y=a[i],h[i].w=180,h[i].h=i<2?270:i<4?200:240;cars=cars.filter(c=>c.dir||c.x<1200||c.x>2e3);ups[2].x=ups[6].x=1630;ups[2].y=635;ups[6].y=1225}",'canal blockers');
s=swap(s,"msg='HARBOR • OPEN QUAYS'","msg='CANAL QUARTER'",'canal framing');
...
s=swap(s,"y:[630,1225,920,1500][cleaners.length%4]","y:zone===1?[635,1225][cleaners.length%2]:[630,1225,920,1500][cleaners.length%4]",'bridge cleaner lanes');fs.writeFileSync(f,s);
...
s=swap(s,"if(!e('ups[2].x===1630&&ups[6].x===1630'))throw Error('harbor promenade powerups moved');","if(!e('ups[2].x===1630&&ups[2].y===635&&ups[6].x===1630&&ups[6].y===1225'))throw Error('bridge powerups moved');if(!e('cleaners.every(c=>c.y===635||c.y===1225)'))throw Error('cleaners must use bridge lanes');",'bridge traffic contract');
s=swap(s,"if(e('msg')!=='HARBOR • OPEN QUAYS')throw Error('Washwater launch framing missing');","if(e('msg')!=='CANAL QUARTER')throw Error('canal launch framing missing');let vx=e(\"(()=>{let u=unis[0],o=objs.find(o=>o.t==='hedge');u.x=1600;u.y=100;u.vx=u.vy=0;collide(u,o,.1);return u.vx})()\");if(vx>=0)throw Error('canal must block unicorn crossing away from bridges');",'canal collision contract');
...
f='scripts/graphics-v041-smoke.mjs';s=fs.readFileSync(f,'utf8');s=swap(s,"msg='HARBOR • OPEN QUAYS'","msg='CANAL QUARTER'",'graphics framing contract');fs.writeFileSync(f,s);
