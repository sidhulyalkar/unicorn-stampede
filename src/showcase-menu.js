// Showcase v1.10: accessible, structured title menu with persistent mastery context.
...
showcaseMenu.innerHTML=`<div class="showcase-menu-card">
  ...
  <div class="showcase-menu-head"><div><h1>BUILD YOUR STAMPEDE</h1><p>Choose a town, set the pressure, then keep six semi-autonomous unicorns useful while everything around them starts falling apart.</p></div><div class="showcase-menu-badge">0 / 12 STARS<br><span>STARTING HERD</span></div></div>
...
function showcaseRefreshMenu(){
  ...
  const mastery=globalThis.showcaseMastery;if(mastery){masteryBadge.innerHTML=`${mastery.total} / 12 STARS<br><span>${mastery.rank}</span>`}
  worldGrid.querySelectorAll('[data-world]').forEach((b,i)=>{const on=i===zone;b.classList.toggle('is-selected',on);b.setAttribute('aria-pressed',String(on));const m=b.querySelector('.showcase-world-mastery'),g=b.querySelector('.showcase-world-goal'),q=mastery?.get(i),stars=q?.stars??Math.max(0,Math.min(3,S.g('ccC'+i)|0));m.textContent=stars?'★'.repeat(stars)+'☆'.repeat(3-stars):'☆☆☆';m.setAttribute('aria-label',stars+' of 3 mastery stars');g.textContent=q?.next==='MASTERED'?'MASTERED':'NEXT • '+(q?.next||'CONQUER')});
