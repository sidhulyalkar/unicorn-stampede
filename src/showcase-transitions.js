...
let showcaseTransitionHandoffs=0,showcasePrisonerResets=0,showcaseRescueResets=0;
function showcaseQuiesceUnicorn(u){
  if(!u)return;u.vx=u.vy=0;u.order=u.dash=u.stun=0;u.ai=0;u.tx=u.x;u.ty=u.y;u.rescue=0;u.frontierSlip=0;
}
...
update=function(dt){
  ...
  for(const id of held){const u=unis[id];if(!u)continue;if(u.vx||u.vy||u.order||u.dash||u.stun||u.frontierSlip)showcasePrisonerResets++;showcaseQuiesceUnicorn(u)}
...
const showcaseTransitionStartBase=startLevel;
startLevel=function(n){const r=showcaseTransitionStartBase(n);return r};
globalThis.showcaseTransitions={quiesce:showcaseQuiesceUnicorn,get stats(){return{handoffs:showcaseTransitionHandoffs,prisonerResets:showcasePrisonerResets,rescueResets:showcaseRescueResets}}};
