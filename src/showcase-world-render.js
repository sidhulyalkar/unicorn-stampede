...
function showcaseWorldPalette(){return SHOWCASE_WORLD_PALETTES[level?zone:0]||SHOWCASE_WORLD_PALETTES[0]}
...
function showcaseBuildSurface(){
  ...
  for(const r of roads){
    ...
    if(level&&zone===2){Q.globalAlpha=.2;Q.fillStyle='#d6dcdf';if(r[2]>r[3])Q.fillRect(r[0]+8,r[1]+12,r[2]-16,8);else Q.fillRect(r[0]+12,r[1]+8,8,r[3]-16);Q.globalAlpha=1}
    Q.strokeStyle=p.lane;Q.lineWidth=3;Q.setLineDash(level&&zone===2?[]:[28,28]);Q.beginPath();if(r[2]>r[3]){Q.moveTo(r[0],r[1]+r[3]/2);Q.lineTo(r[0]+r[2],r[1]+r[3]/2)}else{Q.moveTo(r[0]+r[2]/2,r[1]);Q.lineTo(r[0]+r[2]/2,r[1]+r[3])}Q.stroke();Q.setLineDash([]);
    if(level&&zone===3){Q.globalAlpha=.26;Q.strokeStyle='#37271f';Q.lineWidth=4;Q.beginPath();if(r[2]>r[3]){let y=r[1]+r[3]/2;Q.moveTo(r[0],y-28);Q.lineTo(r[0]+r[2],y-28);Q.moveTo(r[0],y+28);Q.lineTo(r[0]+r[2],y+28)}else{let x=r[0]+r[2]/2;Q.moveTo(x-28,r[1]);Q.lineTo(x-28,r[1]+r[3]);Q.moveTo(x+28,r[1]);Q.lineTo(x+28,r[1]+r[3])}Q.stroke();Q.globalAlpha=1}
  }
  if(level&&!zone)for(let x of[760,1540,2320])for(let y of[540,1130]){Q.fillStyle='rgba(245,240,220,.72)';for(let k=0;k<5;k++){Q.fillRect(x+10+k*34,y+8,18,58);Q.fillRect(x+10+k*34,y+124,18,58)}}
  showcaseSurfaceDirty=0;showcaseSurfaceBuilds++;
}
world=function(){
  X.fillStyle='#0c1422';X.fillRect(0,0,W,H);X.save();X.translate(ox,oy);X.scale(z,z);
  if(showcaseSurfaceDirty)showcaseBuildSurface();X.drawImage(showcaseSurfaceCanvas,0,0);
