...
function harbor(){roads.splice(1,1);roads[0]=[760,0,140,1130];roads[1]=[2320,540,160,1260];objs=objs.filter(o=>o.t!=='hedge');cars=cars.filter(c=>c.dir);ups[2].x=1450;ups[6].x=1810}
...
const _startW=startLevel;startLevel=function(n){seedZone=n?zone:0;if(n)S.s('ccRun',++runN);_startW(n);if(n&&zone===1)harbor();wind=1;windT=2;chains=0;if(n){if(zone===1){addClean(2);for(let c of cleaners)c.v*=1.35;msg='HARBOR • TWO BRIDGES';msgT=2}lmText=ZN[zone];lmTextT=2}};
...
const _worldW=world;world=function(){_worldW();if(!level||!zone)return;X.save();X.translate(ox,oy);X.scale(z,z);if(zone===1){X.strokeStyle='#9de8ff44';for(let i=6;i--;){let y=(i*311+clock*90)%WH;X.beginPath();X.moveTo(1555,y);X.lineTo(1705,y);X.stroke()}}else{X.fillStyle='rgba(245,250,255,.09)';for(let i=0;i<7;i++){let x=(i*510+clock*wind*45)%3500-120,y=180+(i%3)*560;X.beginPath();X.ellipse(x,y,180,70,0,0,T);X.fill()}}X.restore()};
const _titleW=title;