...
const ZN=['PRISMBOROUGH','WASHWATER BAY','CLOUDTOP HEIGHTS'],BG=['#78986d','#74889a'],_day=today;today=()=>_day()+seedZone*173+runN*19;
...
function harbor(){let f=x=>220+x*.82+(x>940?120:0)+(x>1720?100:0)+(x>2500?80:0),g=y=>20+y*.92+(y>730?70:0)+(y>1320?50:0),m=q=>{let x=f(q.x),y=g(q.y),w=q.w&&f(q.x+q.w)-x,h=q.h&&g(q.y+q.h)-y;q.x=x;q.y=y;if(w)q.w=w;if(h)q.h=h};for(let o of objs)m(o);for(let r of roads){let x=f(r[0]),y=g(r[1]);r[2]=f(r[0]+r[2])-x;r[3]=g(r[1]+r[3])-y;r[0]=x;r[1]=y;if(r[2]>r[3])r[0]=0,r[2]=WW;else r[1]=0,r[3]=WH}for(let a of [ups,flowers,unis,people,flies,cars])for(let q of a){q.x=f(q.x);q.y=g(q.y);if(q.tx!=null)q.tx=f(q.tx),q.ty=g(q.ty)}objs=objs.filter(o=>o.t!=='hedge')}
...
const _startW=startLevel;startLevel=function(n){seedZone=n?zone:0;if(n)S.s('ccRun',++runN);_startW(n);if(n&&zone===1)harbor();wind=1;windT=2;chains=0;if(n){if(zone===1){addClean(2);for(let c of cleaners)c.v*=1.35;msg='HARBOR DISTRICT • OPEN LANES';msgT=2}lmText=ZN[zone];lmTextT=2}};
