let zone=S.g('ccZone')%3,runN=S.g('ccRun'),seedZone=0,wind=1,windT=0,chains=0,crowns=0,townFlip=0;
...
function flipTown(a){if(a&1){for(let o of objs)o.x=3260-o.x-o.w;for(let q of ups)q.x=3260-q.x;for(let q of flowers)q.x=3260-q.x;for(let u of unis)u.x=u.tx=3260-u.x,u.a=Math.PI-u.a}if(a&2){for(let o of objs)o.y=1860-o.y-o.h;for(let q of ups)q.y=1860-q.y;for(let q of flowers)q.y=1860-q.y;for(let u of unis)u.y=u.ty=1860-u.y,u.a=-u.a}}
...
const _startW=startLevel;startLevel=function(n){let prev=state,won=landWin;if(n&&prev==='end'&&won){zone=(zone+1)%3;S.s('ccZone',zone)}seedZone=n?zone:0;if(n)S.s('ccRun',++runN);_startW(n);townFlip=n?runN&3:0;if(townFlip)flipTown(townFlip);wind=1;windT=2;chains=crowns=0;if(n){timeLeft=[100,94,90][zone];if(zone===1){addClean(2);for(let c of cleaners)c.v*=1.35}lmText=ZN[zone];lmTextT=2}};
