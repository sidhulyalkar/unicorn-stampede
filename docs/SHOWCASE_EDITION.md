...
- Run telemetry records switches, attention reasons, whips, dashes, power-ups, structure destruction, damage, charge, elapsed time, Flow actions, peak Flow, and route orders.
- Run Intelligence converts those recorded signals into a deterministic `RUN READ` plus one `NEXT EXPERIMENT`. It describes observed coordination, routing, crisis attention, and capture burden without changing score or claiming an optimal strategy.
- The v1.16 lifecycle bridge provides one ordered extension seam for newer cross-cutting systems. Herd Flow and Living Conquest have been migrated from direct lifecycle wrappers onto named hook listeners while the older qualified showcase stack remains intact.
...
- Chromium and Firefox are exercised in CI through gameplay matrices, deterministic long-run stress, capture/rescue transitions, Frontier lifecycle seams, Smart Director decision/readability checks, Herd Flow/crowd-response qualification, Living Conquest authority/readability checks, hook ordering, and run-debrief interpretation.

## Product direction

The website edition should optimize for the strongest playable demonstration, not compressed bytes. After v1.16 the next high-value question is empirical: which visible mastery signals actually correspond to cleaner distributed play? Collect compact human-run summaries, compare Flow, route behavior, attention reasons, capture burden, paint retention, conquest outcome/time, and only then tune balance or grant any additional mechanical weight to Flow. Architecturally, all new cross-cutting showcase work should use the hook bridge; older wrappers should migrate only when their lifecycle surface is actively being changed and parity can be qualified.
